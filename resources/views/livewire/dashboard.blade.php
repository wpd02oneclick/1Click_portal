...
<div class="row">
                    ...
                    <div class="row">
                        <div class="col-8">
                            <div class="mt-0 text-start"><span class="font-weight-semibold">In Leave Employees</span>
                                <h3 class="mb-0 mt-1 text-secondary mb-2">420</h3></div>
                        </div>
                        <div class="col-4">
    ...
    <div class="col-sm-6 col-md-6 col-lg-3">
        <div class="card">
            <div class="card-body"><h2 class="mb-1 font-weight-bold">{{ $Leave_Quota->Annual_Leaves }}</h2>
                <div class="text-muted">Annual Leaves => {{ $auth_user->leaves->Annual_Leaves }}</div>
                <div class="progress progress-sm mt-2">
                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-primary "
                         style="width: {{ PortalHelpers::getLeaveQuotaPercentage($auth_user->leaves->Annual_Leaves, $Leave_Quota->Annual_Leaves) }}%"></div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-md-6 col-lg-3">
        <div class="card">
            <div class="card-body"><h2 class="mb-1 font-weight-bold">{{ $Leave_Quota->Casual_Leaves }}</h2>
                <div class="text-muted">Casual Leaves => {{ $auth_user->leaves->Casual_Leaves }}</div>
                <div class="progress progress-sm mt-2">
                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-warning "
                         style="width: {{ PortalHelpers::getLeaveQuotaPercentage($auth_user->leaves->Casual_Leaves, $Leave_Quota->Casual_Leaves) }}%"></div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-md-6 col-lg-3">
        <div class="card">
            <div class="card-body"><h2 class="mb-1 font-weight-bold">{{ $Leave_Quota->Sick_Leaves }}</h2>
                <div class="text-muted">Sick Leaves => {{ $auth_user->leaves->Sick_Leaves }}</div>
                <div class="progress progress-sm mt-2">
                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-danger "
                         style="width: {{ PortalHelpers::getLeaveQuotaPercentage($auth_user->leaves->Sick_Leaves, $Leave_Quota->Sick_Leaves) }}%"></div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-md-6 col-lg-3">
        <div class="card">
            <div class="card-body"><h2 class="mb-1 font-weight-bold">{{ $Leave_Quota->Un_Paid }}</h2>
        ...
        <div class="panel-body tabs-menu-body table_tabs1 pt-5 p-3 border-0">
            <div class="tab-content">
                <div class="tab-pane p-3" id="tab5">
                    @include('partials.dashboard.previous-dead-line-orders', ['Orders' => $Previous_Order])
                </div>
                <div class="tab-pane active p-3" id="tab6">
                    @include('partials.dashboard.today-dead-line-orders', ['Orders' => $Today_Order])
                </div>
                <div class="tab-pane p-3" id="tab7">
                    @include('partials.dashboard.tomorrow-dead-line-orders', ['Orders' => $Tomorrow_Order])
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modaldemo8">
        <div class="modal-dialog  modal-dialog-centered text-center py-5" role="document">
            <div class="modal-content modal-content-demo "
                 style="background-color: #2B3963;border: 1px solid #2B3963; border-radius:18px;">
                <div class="modal-header">
                    <h6 class="modal-title text-center fs-4 text-white">Mark Attendance</h6>
                    <button aria-label="Close" class="btn-close text-white" data-bs-dismiss="modal"><span
...
@if(!empty($Previous_Order) && !empty($Today_Order) && !empty($Tomorrow_Order))
    <script>
        ...
        const count_id = {{ Js::from($deadline_times) }};
