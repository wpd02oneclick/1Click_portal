    ...
    <table class="table table-vcenter text-nowrap border-top dead-line-orders mb-0">
        ...
        @forelse($Orders as $Order)
            @php
                ...
                if (!empty($Order->submission_info->F_DeadLine) && (\Carbon\Carbon::parse($Order->submission_info->F_DeadLine)->isToday())){
                    $DeadLine_DateAndTime = $Order->submission_info->F_DeadLine.''.$Order->submission_info->DeadLine_Time;
                    $deadlineType = '1st Draft';
                } elseif (!empty($Order->submission_info->S_DeadLine) && (\Carbon\Carbon::parse($Order->submission_info->S_DeadLine)->isToday())){
                    $DeadLine_DateAndTime = $Order->submission_info->S_DeadLine.''.$Order->submission_info->DeadLine_Time;
                    $deadlineType = '2nd Draft';
                } elseif (!empty($Order->submission_info->T_DeadLine) && (\Carbon\Carbon::parse($Order->submission_info->T_DeadLine)->isToday())){
                    $DeadLine_DateAndTime = $Order->submission_info->T_DeadLine.''.$Order->submission_info->DeadLine_Time;
                    $deadlineType = '3rd Draft';
                }
                elseif (!empty($Order->submission_info->DeadLine) && !empty($Order->submission_info->DeadLine_Time) && (\Carbon\Carbon::parse($Order->submission_info->DeadLine)->isToday())){
            ...
            @endphp
            @if(!(\Carbon\Carbon::parse($DeadLine_DateAndTime)->isToday()) || $Order_Status === 'Completed')
                @continue
            @endif
            <tr>
