...
class LoginForm extends Component
{
    ...
    public function UserLogin(Request $request): RedirectResponse
    {
        ...
        if ($user) {
            if (Hash::check($request->Log_Password, $user->password)) {
                session()->put('permission', $user->designation->module_permission);
                session()->put('other_permission', $user->designation->other_permission);
                LoginHistory::create([
                    'user_id' => $user->id,
    ...
    public function UserLogout(): RedirectResponse
    {
        ...
        session()->forget('other_permission');
